...
import { stockIn } from '@/features/inventory/services/inventory.service';
import type { StockInLine, StockLocation } from '@/features/inventory/types';
import type { Product } from '@/types/database.types';

type StockInLineDraft = StockInLine & { key: string };
...
function createLine(): StockInLineDraft {
  return {
...
export function StockInForm({ locations, products, defaultLocationId }: StockInFormProps) {
  const queryClient = useQueryClient();
  const [locationId, setLocationId] = useState(defaultLocationId ?? '');
  const [lines, setLines] = useState<StockInLineDraft[]>([createLine()]);
  ...
  const updateLine = (key: string, patch: Partial<StockInLineDraft>) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    for (const line of validLines) {
      if (seen.has(line.product_id)) {
        setErrorMessage('Each product can only appear once per stock in.');
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <h3 className="text-lg font-semibold text-slate-900">Stock In</h3>
